...
SerialData::SerialData(const BasicVQFParams& params, vqf_real_t gyrTs, vqf_real_t accTs, vqf_real_t magTs)
{
    this->params = params;
...
void SerialData::updateGyr(const vqf_real_t gyr[3])
{
...
void SerialData::updateAcc(const vqf_real_t acc[3])
{
...
void SerialData::updateMag(const vqf_real_t mag[3])
{
    // ignore [0 0 0] samples
...
void SerialData::update(const vqf_real_t gyr[3], const vqf_real_t acc[3])
{
    updateGyr(gyr);
    updateAcc(acc);
}

void SerialData::update(const vqf_real_t gyr[3], const vqf_real_t acc[3], const vqf_real_t mag[3])
{
    updateGyr(gyr);
    updateAcc(acc);
    updateMag(mag);
...
void SerialData::updateBatch(const vqf_real_t gyr[], const vqf_real_t acc[], const vqf_real_t mag[], size_t N,
...
void SerialData::getQuat3D(vqf_real_t out[4]) const
{
    std::copy(state.gyrQuat, state.gyrQuat + 4, out);
}

void SerialData::getQuat6D(vqf_real_t out[4]) const
{
    quatMultiply(state.accQuat, state.gyrQuat, out);
}

void SerialData::getQuat9D(vqf_real_t out[4]) const
{
    quatMultiply(state.accQuat, state.gyrQuat, out);
    quatApplyDelta(out, state.delta, out);
}

vqf_real_t SerialData::getDelta() const
{
    return state.delta;
}

void SerialData::setTauAcc(vqf_real_t tauAcc)
{
...
void SerialData::setTauMag(vqf_real_t tauMag)
{
    params.tauMag = tauMag;
    coeffs.kMag = gainFromTau(params.tauMag, coeffs.magTs);
}

const BasicVQFParams& SerialData::getParams() const
{
    return params;
}

const BasicVQFCoefficients& SerialData::getCoeffs() const
{
    return coeffs;
}

const BasicVQFState& SerialData::getState() const
{
    return state;
}

void SerialData::setState(const BasicVQFState& state_)
{
    this->state = state_;
}

void SerialData::resetState()
{
...
void SerialData::quatMultiply(const vqf_real_t q1[4], const vqf_real_t q2[4], vqf_real_t out[4])
{
...
void SerialData::quatConj(const vqf_real_t q[4], vqf_real_t out[4])
{
    vqf_real_t w = q[0];
    vqf_real_t x = -q[1];
    vqf_real_t y = -q[2];
...
void SerialData::quatSetToIdentity(vqf_real_t out[4])
{
    out[0] = 1;
    out[1] = 0;
    out[2] = 0;
...
void SerialData::quatApplyDelta(vqf_real_t q[], vqf_real_t delta, vqf_real_t out[])
{
...
void SerialData::quatRotate(const vqf_real_t q[4], const vqf_real_t v[3], vqf_real_t out[3])
{
...
vqf_real_t SerialData::norm(const vqf_real_t vec[], size_t N)
{
    vqf_real_t s = 0;
    for (size_t i = 0; i < N; i++) {
        s += vec[i] * vec[i];
...
void SerialData::normalize(vqf_real_t vec[], size_t N)
{
...
void SerialData::clip(vqf_real_t vec[], size_t N, vqf_real_t min, vqf_real_t max)
{
    for (size_t i = 0; i < N; i++) {
        if (vec[i] < min) {
            vec[i] = min;
...
vqf_real_t SerialData::gainFromTau(vqf_real_t tau, vqf_real_t Ts)
{
    assert(Ts > 0);
...
void SerialData::filterCoeffs(vqf_real_t tau, vqf_real_t Ts, double outB[], double outA[])
{
    assert(tau > 0);
    assert(Ts > 0);
...
void SerialData::filterInitialState(vqf_real_t x0, const double b[3], const double a[2], double out[])
{
...
void SerialData::filterAdaptStateForCoeffChange(vqf_real_t last_y[], size_t N, const double b_old[],
...
vqf_real_t SerialData::filterStep(vqf_real_t x, const double b[3], const double a[2], double state[2])
{
...
void SerialData::filterVec(const vqf_real_t x[], size_t N, vqf_real_t tau, vqf_real_t Ts, const double b[3],
